...
function ProfileImg() {
  ...
  return (
    <form className='user-photo' encType='multipart/form-data'>
      <label htmlFor='photoUser' className='user-photo__download'>
        <input type='file' name='photoUser' id='photoUser' onChange={handleFileChange} />

        <div className='user-photo__controls'>
          <ControlButton id='edit' />
        </div>
        {previewImage && previewImage !== '' && <img src={previewImage} alt='Preview' />}
      </label>
      <div className='user-photo__controls' onClick={deletePhoto}>
        <ControlButton id='delete' />
      </div>
      {errorDownload && <p className='user-photo__error'>{errorDownload}</p>}
    </form>
  );
}
