...
const BigCalendar = () => {
  ...
  const eventStyleGetter = (event: TEvent) => {
    ...
    const className = classNames('primaryEvent', {
      firstEvent: isFirstHall,
      secondaryEvent: isSecondaryHall,
      thirdEvent: isThirdHall
    });
  ...
  return (
    <>
      ...
      />
      {showDetail && (
        <CalendarItemDetail selectedEvent={selectedEvent} onClick={toggleMenu} />
      )}
    </>
  );
};
